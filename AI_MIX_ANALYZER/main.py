...
class App(tk.Tk):
 def __init__(self):
  ...
  for text,cmd in [('UPLOAD FILES',self.load),('ANALYZE ALL',self.analyze),('EVIDENCE JSON',self.brief),('EXPORT JSON',self.ejson),('EXPORT PDF',self.epdf),('EXPORT CSV',self.ecsv)]: tk.Button(bar,text=text,command=cmd,bg='#263449',fg='white',relief='flat',padx=15,pady=9).pack(side='left',padx=3)
 ...
 def show(self,d):
  ...
  f,l,t,q,s=d['file'],d['loudness'],d['true_peak'],d['quality'],d['stereo']; vals=[('DURATION',f"{f['duration_s']:.2f}s"),('LUFS',f"{l['integrated_lufs']:.1f}"),('TRUE PEAK',f"{t['dbtp']:.2f} dBTP"),('RMS',f"{q['rms_db']:.2f} dBFS"),('CORRELATION',f"{s.get('correlation',1):.2f}"),('CLIPS',str(q['clipped_samples']))]
