...
class App(tk.Tk):
 def __init__(self):
  ...
  for text,cmd in [('UPLOAD FILES',self.load),('ANALYZE ALL',self.analyze),('EVIDENCE JSON',self.brief),('EXPORT JSON — ALL STEMS',self.ejson)]: tk.Button(bar,text=text,command=cmd,bg='#263449',fg='white',relief='flat',padx=15,pady=9).pack(side='left',padx=3)
  self.progress=ttk.Progressbar(self,length=380); self.progress.pack(pady=8)
  top=tk.Frame(self,bg=BG); top.pack(fill='x',padx=18)
  tk.Label(top,text='UPLOADED FILES',fg=MUTED,bg=BG,font=('Segoe UI',9,'bold')).pack(anchor='w'); self.files=tk.Listbox(top,height=4,bg='#171e28',fg=TEXT,selectbackground='#36506d',relief='flat'); self.files.pack(fill='x'); self.files.bind('<<ListboxSelect>>',self.select)
  rolebar=tk.Frame(top,bg=BG); rolebar.pack(fill='x',pady=4); tk.Label(rolebar,text='ROLE FOR SELECTED FILE:',fg=MUTED,bg=BG).pack(side='left'); self.role=ttk.Combobox(rolebar,values=['Other','Kick','Snare','Drums','Bass','Lead Vocal','Backing Vocal','Guitar','Piano','Synth','FX'],state='readonly',width=18); self.role.set('Other'); self.role.pack(side='left',padx=8); tk.Button(rolebar,text='ASSIGN ROLE',command=self.assign_role,bg='#263449',fg='white',relief='flat').pack(side='left')
 ...
 def draw_graph(self):
  ...
  c=self.graph;c.delete('all');w=max(c.winfo_width(),500);h=max(c.winfo_height(),190);left,bottom=55,28;mode=self.graph_mode.get();vals=[];label=mode
  if mode=='Loudness':vals=self.data['loudness'].get('history',[]);label='LUFS history'
   ...
   corr=self.data['stereo'].get('correlation',0);vals=[corr.get('overall',0) if isinstance(corr,dict) else corr];label='Stereo correlation'
  if not vals:vals=[0]
 ...
 def ejson(self):
  ...
  if not done:return messagebox.showinfo('Export JSON','Analyze the loaded stems first.')
  ...
  for source_path,d in done.items():
   try:
    ...
    used.add(safe.lower());export_json(d,os.path.join(folder,f'{safe}.json'));exported.append(f'{safe}.json')
  ...
  else:messagebox.showinfo('JSON export complete',f'Exported {len(exported)} stem JSON file(s) to:\n{folder}')
