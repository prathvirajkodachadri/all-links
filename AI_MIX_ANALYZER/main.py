...
class App(tk.Tk):
    ...
    def _build_inputs(self):
        ...
        self.files = tk.Listbox(stems, height=5, bg='#171e28', fg=TEXT, selectbackground='#36506d', relief='flat', exportselection=False)
        self.files.pack(fill='x', pady=4); self.files.bind('<<ListboxSelect>>', self.select)
        ...
        rb = tk.Frame(stems, bg=PANEL); rb.pack(fill='x', pady=(5, 0))
        tk.Label(rb, text='ROLE:', fg=MUTED, bg=PANEL).pack(side='left')
        self.role = ttk.Combobox(rb, values=ROLES, state='readonly', width=16); self.role.set('Other'); self.role.pack(side='left', padx=6)
    ...
    def _build_analysis(self):
        ...
        graphbar = tk.Frame(self, bg=BG); graphbar.pack(fill='x', padx=18)
        ...
        self.out = tk.Text(body, bg='#0b0e12', fg=TEXT, font=('Consolas', 9), relief='flat', padx=12, pady=10); self.out.pack(side='left', fill='both', expand=True)
        self.issues = tk.Text(body, width=42, bg='#171e28', fg=TEXT, font=('Segoe UI', 10), relief='flat', padx=12, pady=10); self.issues.pack(side='right', fill='y', padx=(10, 0))
        self.issues.insert('end', 'Project checks will appear after analysis.'); self.issues.config(state='disabled')
    ...
    def _update_export_buttons(self):
        ...
        self.chatgpt_button.configure(state=state); self.stem_export_button.configure(state=state)
        self.analyze_button.configure(state='disabled' if self.analyzing else 'normal')

    def _selected_file_check(self, path, label):
        """Fail early with a useful message if Windows cannot access a selected source."""
        if not os.path.isfile(path):
            messagebox.showerror('Audio file not accessible', f'{label} cannot be accessed by this Python process:\n\n{path}\n\nIf this is a mapped/network drive, try the folder using its UNC path (\\\\server\\share...) or run the analyzer under the same Windows account/session that owns the drive mapping.')
            return False
        return True
    ...
    def remove_mix(self):
        self.mix_path = None; self.mix_label.config(text='No mix selected', fg=MUTED); self._invalidate_analysis()
    ...
    def load_stems(self):
        ...
        added = 0; inaccessible = []
        for p in paths:
            if not os.path.isfile(p): inaccessible.append(p); continue
        ...
        if inaccessible:
            messagebox.showwarning('Some audio files were not accessible', 'These files were not added because Windows/Python could not access them:\n\n' + '\n'.join(inaccessible))
    ...
    def clear_stems(self):
        self.results.clear(); self.roles.clear(); self.files.delete(0, 'end'); self._invalidate_analysis(); self.status.set('All stems cleared.')
    ...
    def finish_project(self):
        ...
        if failed:
            self.analysis_complete = False; self._update_export_buttons(); self.status.set('Analysis incomplete. Fix the failed file(s) and run Analyze Project again.')
        ...
        self.project_analysis = analyze_project(mix, stems, self.reference_data); self.project_analysis['priority_actions'] = priority_actions(mix, stems, self.project_analysis)
        self.data = mix; self.analysis_complete = True; self._update_export_buttons(); self.show(mix); self._show_project_checks(self.project_analysis)
    ...
    def draw_graph(self):
        ...
        if not vals: vals = [0]
        lo, hi = min(vals), max(vals); lo -= .05 * max(abs(lo), 1); hi += .05 * max(abs(hi), 1)
    ...
    def export_for_chatgpt(self):
        ...
        try:
            zip_path, root = export_chatgpt_package(folder, self.mix_path, self.mix_data, self.results, self.reference_path, self.reference_data)
            self.status.set('ChatGPT package exported: ' + zip_path)
            messagebox.showinfo('ChatGPT export complete', f'Created:\n{os.path.basename(zip_path)}\n\nFolder:\n{root}')
