...
def compare_spectral_masking(a, b, sr):
    ...
    sa=a['spectrum']; sb=b['spectrum']; fa=np.asarray(sa['frequencies_hz']); ma=10**(np.asarray(sa['magnitude'],dtype=float)/20); mb=10**(np.asarray(sb['magnitude'],dtype=float)/20); ma[~np.isfinite(ma)]=0; mb[~np.isfinite(mb)]=0
